...
function show(node, message) { node.textContent = message; }
 $('login').onclick = async () => { try { const r = await fetch(`${API}/auth/login`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:$('email').value, password:$('password').value})}); const data = await r.json(); if (!r.ok) { const detail=Array.isArray(data.detail)?data.detail.map(item=>item.msg||'Invalid value').join('. '):data.detail; return show($('message'), detail || 'Could not connect'); } await chrome.storage.local.set({token:data.access_token}); init(); } catch { show($('message'), 'CareerFlow is unreachable. Start the backend first.'); } };
