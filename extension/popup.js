...
function show(node, message) { node.textContent = message; }
 $('login').onclick = async () => { const r = await fetch(`${API}/auth/login`, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({email:$('email').value, password:$('password').value})}); const data = await r.json(); if (!r.ok) { const detail=Array.isArray(data.detail)?data.detail.map(item=>item.msg||'Invalid value').join('. '):data.detail; return show($('message'), detail || 'Could not connect'); } await chrome.storage.local.set({token:data.access_token}); init(); };
$('auto').onchange = () => chrome.storage.local.set({autoCapture: $('auto').checked});
$('save').onclick = async () => { const result = await new Promise(resolve => chrome.runtime.sendMessage({type:'capture-job', job}, resolve)); show($('message2'), result.ok ? (result.duplicate ? 'Already saved.' : 'Saved to CareerFlow ✓') : result.reason); };
